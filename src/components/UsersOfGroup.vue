...
const headers = computed(() => {
  let h = [...baseHeaders]
  if (members.value.length && members.value[0].role) h.push({ key: 'role', title: 'Role', sortable: false })
  if (props.currentUserHasAdminRights) h = [...h, ...adminHeaders]
  return h
})

async function loadMembers({ page, itemsPerPage }: { page: number; itemsPerPage: number }) {
  loading.value = true
  try {
    const { data } = await api.get(`/v3/groups/${groupId.value}/${props.user_type}`, {
      params: {
...
async function removeUser(user: any) {
  if (!props.currentUserHasAdminRights) return alert('This action can only be performed by group administrators')
